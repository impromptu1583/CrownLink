...
private:
    ...
    void load_field(Json& json, const std::string& key, T& out_value) {
        try {
            ...
            // spdlog::warn("Value for \"{}\" not found (using default: {}), exception: {}", key, as_string(out_value),
            // ex.what());
            spdlog::warn("Value for \"{}\" not found (using default), exception: {}", key, ex.what());
        } catch (const Json::type_error& ex) {
            // spdlog::warn("Value for \"{}\" is of wrong type (using default: {}), exception: {}", key,
            // as_string(out_value), ex.what());
